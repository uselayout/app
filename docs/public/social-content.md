...
### Launch Announcement
Introducing Layout — paste a Figma URL or any website, get a structured layout.md that makes Claude, Cursor, and Codex generate on-brand UI every time. No plugins. No manual tokens. 2 minutes.

layout.design

### Launch Thread (Build Story)

1/ I built a tool that turns any design system into AI-ready context. Paste a Figma URL or live website → get a layout.md + tokens.css + CLAUDE.md + AGENTS.md. Here's why and how:
...
4/ How it works: Paste a Figma file URL → AI Studio calls the Figma API, extracts every colour, text style, effect, and component → Claude synthesises a structured layout.md → export as a ZIP with 7 formats.
...
**Website Extraction**:
No Figma file? No problem. Layout extracts design tokens from any live website — fonts, colours, spacing, shadows — all from computed CSS. Paste stripe.com, get Stripe's design system as layout.md. layout.design

**Export Bundle**:
One extraction. Seven formats. layout.md, CLAUDE.md, AGENTS.md, .cursorrules, tokens.css, tokens.json, tailwind.config.js. Works with Claude Code, Cursor, Copilot, Windsurf, and Codex. layout.design
...
**Before/After**:
Left: AI output without layout.md — generic colours, wrong fonts, inconsistent spacing.
Right: AI output with layout.md — exact brand colours, correct typography, proper component patterns.
...
- Generates a structured layout.md optimised for LLM consumption
...
So I built the connection. Layout extracts design tokens from Figma (or any live website) and compiles them into structured layout.md files. Drop it in your repo, and every AI prompt has full design context.

The hardest part wasn't the extraction — it was getting the layout.md format right. Too much detail and you blow the context window. Too little and the AI still generates generic output. The Quick Reference section (50–75 lines) turned out to be the sweet spot.
...
**The solution**: Paste a Figma URL or any website URL. AI Studio extracts colours, typography, spacing, components, and synthesises a structured layout.md.

**Export bundle includes**: layout.md, CLAUDE.md, AGENTS.md, .cursorrules, tokens.css, tokens.json, tailwind.config.js.
...
Would love feedback on the layout.md format — particularly the Quick Reference section and how well it works with different AI tools.
...
Paste a URL. Get a layout.md, CLAUDE.md, AGENTS.md, tokens.css, and tailwind.config.js in under 2 minutes.
...
2. The Quick Reference format — a 50-75 line summary inside layout.md that fits in any context window without blowing your token budget.
...
- **The fix**: A structured layout.md with Quick Reference, colour system, typography, components, and anti-patterns.
...
- **The layout.md**: Publish the actual output (edited for clarity).
