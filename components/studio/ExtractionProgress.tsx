            ...
            {warnings && warnings.length > 0 && (
              <div className="space-y-3 mt-4">
                {warnings.map((warning, i) => (
                  <div key={i} className="flex items-start gap-2 rounded-md border border-[var(--status-warning)]/30 bg-[var(--status-warning)]/10 px-3 py-2">
                    <AlertTriangle className="h-4 w-4 text-[var(--status-warning)] flex-shrink-0 mt-0.5" />
                    <span className="text-xs text-[var(--status-warning)]">{warning}</span>
                  </div>
                ))}
